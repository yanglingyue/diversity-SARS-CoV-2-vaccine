...
#Rename the Shannon's index of lineage diversity data, calculated based on subsampling strategy 2, as "sub_lineage"
colnames(dat1)[20] <- "sub_lineage" 

#Rename the richness of lineage diversity data, calculated based on subsampling strategy 2, as "sub_richness"
colnames(dat1)[21] <- "sub_richness" 

#Rename the evenness of lineage diversity data, calculated based on subsampling strategy 2, as "sub_evenness"
colnames(dat1)[22] <- "sub_evenness" 

#Rename the Simpson' indec of lineage diversity data, calculated based on subsampling strategy 1
...
dir.create("02_fig/figs7_strategy2")
outpath <-  "02_fig/figs7_strategy2/strategy2_figs7"
...
Y2  <- data$sub_lineage # response variable (Shannon's index of lineage diversity based on the subsampling strategy 2)
Y3  <- data$sub_richness # response variable (the richness of lineage diversity based on the subsampling strategy 2)
Y4  <- data$sub_evenness # response variable (the evenness of lineage diversity based on the subsampling strategy 2)
...
#pdf("02_fig/figs6-per-200-a-f.pdf", width = 21, height = 21/3*2)
...
plot(varby, gr[,lag_gr], col = col1, type = "l", lwd = 3, 
     xlab = "International travel", ylab = "", main = "", 
     ylim = range(-4, 16), frame.plot = F, axes = F,family = "ArialN",cex.lab=2.8,line =5)
axis(1,at=0:5*3,cex.axis=2.8,family = "ArialN",mgp=c(3,2,0))
axis(2,at=c(-1:0*4,4:0*4),cex.axis=2.8,family = "ArialN",mgp=c(3,1,0))
xx <- c(varby, rev(varby))
yy <- c(gr.lci[,lag_gr], rev(gr.uci[,lag_gr]))
polygon(xx, yy, col = tcol1, border = tcol1)
abline(h = 0, lty = 3)
...
mtext(side = 2, at = 18, text = "C", las = 2, cex = 2.5, family = "ArialN",line = 5.5)
...
plot(varby, gr[,lag_gr], col = col1, type = "l", lwd = 3, 
     xlab = "Stringency index", ylab = "", main = "", 
     ylim = range(-9, 0), frame.plot = F, axes = F,family = "ArialN",cex.lab=2.8,line =5)
axis(1, at = 0:5*20, labels = 0:5*20,cex.axis=2.8,family = "ArialN",mgp=c(3,2,0))
axis(2,at=c(-3:0*3),cex.axis=2.8,family = "ArialN",mgp=c(3,1,0))
xx <- c(varby, rev(varby))
yy <- c(gr.lci[,lag_gr], rev(gr.uci[,lag_gr]))
polygon(xx, yy, col = tcol1, border = tcol1)
abline(h = 0, lty = 3)
...
polygon(xx, yy, col = tcol2, border = tcol2)
abline(h = 0, lty = 3)
mtext("Effect on growth rate of cases and \nrichness of lineage diversity",side = 2,at=-4.5, line =4,cex=1.9,family = "ArialN")
...
p1  <- ggplot(result_vero_shannon,aes(x=coverage, y=vero_fit,group=coverage))+ 
  ...
  scale_y_continuous(limits = c(-1.1,0.5),breaks = c(-1,-0.5,0,0.5))+
  mytheme+
...
ggsave(file=paste0(outpath,"_figs-g",".pdf"),
