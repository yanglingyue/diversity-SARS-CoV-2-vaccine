...
dir.create("02_fig/figs13_fourth_strategy1")
outpath <-  "02_fig/figs13_fourth_strategy1/fig13_fourth"
...
Y2  <- data$sub_lineage # response variable (Shannon's index of lineage diversity based on the subsampling strategy 1,merging the sub-lineages into lineages according to PANGO nomenclature)
Y3  <- data$sub_richness # response variable (the richness of lineage diversity based on the subsampling strategy 1, merging the sub-lineages into lineages according to PANGO nomenclature)
Y4  <- data$sub_evenness # response variable (the evenness of lineage diversity based on the subsampling strategy 1,merging the sub-lineages into lineages according to PANGO nomenclature)
...
plot(varby, gr[,lag_gr], col = col1, type = "l", lwd = 3, 
     xlab = "International travel", ylab = "", main = "", 
     ylim = range(0, 15), frame.plot = F, axes = F,family = "ArialN",cex.lab=2.8,line =5)
axis(1,at=0:5*3,cex.axis=2.8,family = "ArialN",mgp=c(3,2,0))
axis(2,at=c(3:0*5),cex.axis=2.8,family = "ArialN",mgp=c(3,1,0))
xx <- c(varby, rev(varby))
yy <- c(gr.lci[,lag_gr], rev(gr.uci[,lag_gr]))
polygon(xx, yy, col = tcol1, border = tcol1)
abline(h = 0, lty = 3)
...
polygon(xx, yy, col = tcol2, border = tcol2)
abline(h = 0, lty = 3)
mtext("Effect on growth rate of cases and \nrichness of lineage diversity",side = 2,at=7.5, line =4,cex=1.9,family = "ArialN")
...
#       lwd = 3, lty = 1, bty = "n", 
#       y.intersp = 1.5, horiz = F,cex=2.4)
mtext(side = 2, at = 16.8, text = "C", las = 2, cex = 2.5, family = "ArialN",line = 5.5)
...
plot(varby, gr[,lag_gr], col = col1, type = "l", lwd = 3, 
     xlab = "Stringency index", ylab = "", main = "", 
     ylim = range(-15, 0), frame.plot = F, axes = F,family = "ArialN",cex.lab=2.8,line =5)
axis(1, at = 0:5*20, labels = 0:5*20,cex.axis=2.8,family = "ArialN",mgp=c(3,2,0))
axis(2,at=c(-3:0*5),cex.axis=2.8,family = "ArialN",mgp=c(3,1,0))
xx <- c(varby, rev(varby))
yy <- c(gr.lci[,lag_gr], rev(gr.uci[,lag_gr]))
...
#    lwd = 3, lty = 1, bty = "n", 
#       y.intersp = 1.5, horiz = F,cex=2.4)
mtext(side = 2, at = 4.5, text = "F", las = 2, cex = 2.5, family = "ArialN",line = 5.5)
