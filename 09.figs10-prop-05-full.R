model_gr <-  glm(GR_NCM_log_diff ~ npi_gr_cb+FI_gr_cb+lineage_gr_cb+travel_gr_cb+NPI:F_I,family=gaussian,data=data)
summary(model_gr)

model_si <-glm(sub_lineage ~ Lag(data$sub_lineage,1)+travel_si_cb+gr_cb+npi_si_cb+FI_si_cb+NPI:F_I,family=gaussian,data=data)
summary(model_si)
...
model_gr_rich <- glm(GR_NCM_log_diff ~ FI_gr_cb+richness_gr_cb+npi_gr_cb+travel_gr_cb+NPI:F_I,family=gaussian,data=data)
summary(model_gr_rich)

model_si_rich <- glm(sub_richness ~ Lag(data$sub_richness,1)+travel_si_cb+npi_si_cb+gr_cb+FI_si_cb+NPI:F_I,family=gaussian,data=data)
...
model_gr_even <- glm(GR_NCM_log_diff ~ FI_gr_cb+evenness_gr_cb+npi_gr_cb+travel_gr_cb+NPI:F_I,family=gaussian,data=data)
summary(model_gr_even)

model_si_even <- glm(sub_evenness ~ Lag(data$sub_evenness,1)+travel_si_cb+npi_si_cb+gr_cb+FI_si_cb+NPI:F_I,family=gaussian,data=data)
...
pdf("02_fig/figs10-a-f.pdf", width = 21, height = 21/3*2)
...
pdf(file="02_fig/figs10_g.pdf",width=7.5,height=7.5)
par(mar = c(5, 4.5, 4, 3))
...
filled.contour(x= seq(0, 3, length.out = nrow(z)),
               ...
               key.axes=axis(4,family = "ArialN",cex.axis=1.8,las=1,seq(-8,4,by=4)),
...
pdf(file="02_fig/figs10_h.pdf",width=7.5,height=7.57)
par(mar = c(5, 4.5, 4, 3))
...
pdf(file="02_fig/figs10_i.pdf",width=7.65,height=8.4)
...
mtext("Effect on Shannon's index\n of lineage diversity",side = 2,at=-10, line =4,cex=1.9,family = "ArialN")
