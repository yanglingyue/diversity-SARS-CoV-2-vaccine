...
S <- data$NI_diff # The difference in natural immunity by six months after infection

df <- data.frame(Y1, Y2, Y3, Y4, P, V, N, C, M)



model_gr <-  gam(Y1 ~ npi_gr_cb+FBW_gr_cb+lineage_gr_cb+travel_gr_cb+S+N+s(C, bs = "re")+P:V
                 ,method = "REML",data=df)
summary(model_gr)
#AIC(model_gr)
...
model_si <-gam(Y2 ~ Lag(df$Y2,1)+travel_si_cb+gr_cb+npi_si_cb+FBW_si_cb+S+N+s(C, bs = "re")+P:V,
               method = "REML",data=df)
summary(model_si)
...
model_gr_rich <- gam(Y1 ~ FBW_gr_cb+richness_gr_cb+npi_gr_cb+travel_gr_cb+S+N+s(C, bs = "re")+P:V,
                     method = "REML",data=df)
...
model_si_rich <- gam(Y3 ~ Lag(df$Y3,1)+travel_si_cb+npi_si_cb+gr_cb+FBW_si_cb+S+N+s(C, bs = "re")+P:V,
                     method = "REML",data=df)
...
model_gr_even <- gam(Y1 ~ FBW_gr_cb+evenness_gr_cb+npi_gr_cb+travel_gr_cb+S+N+s(C, bs = "re")+P:V,
                     method = "REML",data=df)
...
model_si_even <- gam(Y4 ~ Lag(df$Y4,1)+travel_si_cb+npi_si_cb+gr_cb+FBW_si_cb+S+N+s(C, bs = "re")+P:V,
...
pdf("02_fig/figs10-diff-a-f.pdf", width = 21, height = 21/3*2)
...
mtext(side = 2, at = 2.42, text = "A", las = 2, cex = 2.5, family = "ArialN",line = 5.5)
...
#       lwd = 3, lty = 1, bty = "n", 
#       y.intersp = 1.5, horiz = F,cex=2.4)
mtext(side = 2, at = 1.42, text = "B", las = 2, cex = 2.5, family = "ArialN",line = 5.5)
...
#       lwd = 3, lty = 1, bty = "n", 
#       y.intersp = 1.5, horiz = F,cex=2.4)
mtext(side = 2, at = 27, text = "C", las = 2, cex = 2.5, family = "ArialN",line = 5.5)
...
pdf(file="02_fig/figs10-diff-g.pdf",width=7.5,height=7.5)
par(mar = c(5, 4.5, 4, 3))
...
pdf(file="02_fig/figs10-diff-h.pdf",width=7.5,height=7.5)
par(mar = c(5, 4.5, 4, 3))
...
filled.contour(x= seq(0, 3, length.out = nrow(z)),
               ...
               family = "ArialN",cex.main=2,cex.lab=1.8,
               key.axes=axis(4,family = "ArialN",cex.axis=1.8,las=1,seq(-3.2,0.3,by=0.7)),
               plot.axes = { axis(1,cex.axis=1.8, at = 0:nlag, c(0:nlag),family = "ArialN") 
                 axis(2,cex.axis=1.8,family = "ArialN",las=3) })
abline(h = 70, lty = 5,lwd=2.5)
...
pdf(file="02_fig/figs10-diff-i.pdf",width=7.65,height=8.4)
...
mtext("Effect on Shannon's index of lineage diversity",side = 2,at=-7, line =3,cex=1.8,family = "ArialN")
