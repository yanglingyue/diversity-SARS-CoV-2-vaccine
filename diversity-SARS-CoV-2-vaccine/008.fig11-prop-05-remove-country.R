...
#########zero-COVID-19##############
x1 <- which(data$country=="China") 
x2 <- which(data$country=="Singapore") 
x3 <- which(data$country=="South Korea") 
x4 <- which(data$country=="Hong Kong") 
x5 <- which(data$country=="Australia") 
x6 <- which(data$country=="New Zealand") 

#########Afirca##############
x7 <- which(data$country=="Egypt") 
x8 <- which(data$country=="Ghana") 
x9 <- which(data$country=="Nigeria") 
x10 <- which(data$country=="Kenya") 
x11 <- which(data$country=="South Africa") 

data <-data[-c(x1,x2,x3,x4,x5,x6,x7,x8,x9,x10,x11),]
...
plot(varby, gr[,lag_gr], col = col1, type = "l", lwd = 3, 
     xlab = "International travel", ylab = "", main = "", 
     ylim = range(0, 30), frame.plot = F, axes = F,family = "ArialN",cex.lab=2.8,line =5)
axis(1,at=0:5*3,cex.axis=2.8,family = "ArialN",mgp=c(3,2,0))
axis(2,at=c(3:0*10),cex.axis=2.8,family = "ArialN",mgp=c(3,1,0))
xx <- c(varby, rev(varby))
yy <- c(gr.lci[,lag_gr], rev(gr.uci[,lag_gr]))
polygon(xx, yy, col = tcol1, border = tcol1)
abline(h = 0, lty = 3)
...
polygon(xx, yy, col = tcol2, border = tcol2)
abline(h = 0, lty = 3)
mtext("Effect on growth rate of cases and \nrichness of lineage diversity",side = 2,at=15, line =4,cex=1.9,family = "ArialN")
...
#       lwd = 3, lty = 1, bty = "n", 
#       y.intersp = 1.5, horiz = F,cex=2.4)
mtext(side = 2, at = 32.5, text = "C", las = 2, cex = 2.5, family = "ArialN",line = 5.5)
...
plot(varby, gr[,lag_gr], col = col1, type = "l", lwd = 3, 
     xlab = "Stringency index", ylab = "", main = "", 
     ylim = range(-18, 6), frame.plot = F, axes = F,family = "ArialN",cex.lab=2.8,line =5)
axis(1, at = 0:5*20, labels = 0:5*20,cex.axis=2.8,family = "ArialN",mgp=c(3,2,0))
axis(2,at=c(-3:0*6,1:0*6),cex.axis=2.8,family = "ArialN",mgp=c(3,1,0))
xx <- c(varby, rev(varby))
yy <- c(gr.lci[,lag_gr], rev(gr.uci[,lag_gr]))
polygon(xx, yy, col = tcol1, border = tcol1)
abline(h = 0, lty = 3)
...
legend("bottomleft",
    ...
    lwd = 3, lty = 1, bty = "n", 
       y.intersp = 1.5, horiz = F,cex=2.4)
mtext(side = 2, at = 9, text = "F", las = 2, cex = 2.5, family = "ArialN",line = 5.5)
...
pdf(file="02_fig/figs11-remove-country_g.pdf",width=7.5,height=7.5)
par(mar = c(5, 4.5, 4, 3))
...
pdf(file="02_fig/figs11-remove-country-h.pdf",width=7.5,height=7.5)
par(mar = c(5, 4.5, 4, 3))
...
filled.contour(x= seq(0, 3, length.out = nrow(z)),
               ...
               family = "ArialN",cex.main=2,cex.lab=1.8,
               key.axes=axis(4,family = "ArialN",cex.axis=1.8,las=1,seq(-4,0.2,by=0.7)),
               plot.axes = { axis(1,cex.axis=1.8, at = 0:nlag, c(0:nlag),family = "ArialN") 
                 axis(2,cex.axis=1.8,family = "ArialN",las=3) })
abline(h = 70, lty = 5,lwd=2.5)
...
pdf(file="02_fig/figs11-remove-country-i.pdf",width=7.65,height=8.4)
...
# Plot
#par(mar = c(5, 4, 4, 4))
plot(gr_low_cp,"slices",lag=3,cumul=T,col="black",ylim=c(-27,9),xlim=c(-5,10),axes=F,ann=F,lwd=3,cex.axis=1.9,
...
axis(2,at=c(-3:0*9,1:0*9),cex.axis=1.8,family = "ArialN",las=3,mgp=c(3,1,0))
...
mtext("Effect on Shannon's index of lineage diversity",side = 2,at=-9, line =3,cex=1.8,family = "ArialN")
