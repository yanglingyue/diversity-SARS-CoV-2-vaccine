...
model_gr <- glm(GR_NCM_log_diff ~ FBW_gr_cb+evenness_gr_cb+npi_gr_cb+travel_gr_cb+NPI:F_I_BWP,family=gaussian,data=data)

summary(model_gr)

model_si <- glm(sub_evenness ~ Lag(data$sub_evenness,1)+travel_si_cb+npi_si_cb+gr_cb+FBW_si_cb+NPI:F_I_BWP,family=gaussian,data=data)
...
pdf("02_fig/prop-05/fig-s5-a-c.pdf", width = 18, height = 6)
...
# Plot
#par(mar = c(5, 4, 4, 4))
plot(gr_low_cp,"slices",lag=3,cumul=T,col="black",ylim=c(-4,8),xlim=c(-5,10),axes=F,ann=F,lwd=3,cex.axis=5,
...
axis(2,at=c(-1:0*4,2:0*4),cex.axis=2.5,family = "ArialN",mgp=c(3,1,0))
...
mtext("Effect on evenness of lineage",side = 2,at=2, line =4,cex=1.7,family = "ArialN")

mtext(side = 2, at = 10, text = "A", las = 2, cex = 2.5, family = "ArialN",line = 5.5)
...
plot(varby, gr[,lag_gr], col = col1, type = "l", lwd = 3, 
     ...
     ylim = range(-1, 1), frame.plot = F, axes = F,family = "ArialN",cex.lab=2.5,line =5)
axis(1,at=0:5*3,cex.axis=2.5,family = "ArialN",mgp=c(3,2,0))
axis(2,at=c(-1:0*1,1:0*1),cex.axis=2.5,family = "ArialN",mgp=c(3,1,0))
...
mtext("Effect on growth rate and \n evenness of lineage",side = 2,at=0, line =4,cex=1.7,family = "ArialN")

legend("bottomleft",
       legend = c(paste0("Evenness of lineage"),
                  
                  paste0("Growth rate")),
       col = c(col2, col1), 
       lwd = 3, lty = 1, bty = "n", 
       y.intersp = 1.5, horiz = F,cex=2.5)
mtext(side = 2, at = 1.3, text = "B", las = 2, cex = 2.5, family = "ArialN",line = 5.5)
...
axis(2,at=c(-2:0*2),cex.axis=2.5,family = "ArialN",mgp=c(3,1,0))
xx <- c(varby, rev(varby))
yy <- c(gr.lci[,lag_gr], rev(gr.uci[,lag_gr]))
polygon(xx, yy, col = tcol1, border = tcol1)
abline(h = 0, lty = 3)
...
polygon(xx, yy, col = tcol2, border = tcol2)
abline(h = 0, lty = 5,lwd=1)
mtext("Effect on growth rate and \n evenness of lineage",side = 2,at=-2, line =4,cex=1.7,family = "ArialN")
...
legend("bottomleft",
       ...
       lwd = 3, lty = 1, bty = "n", 
       y.intersp = 1.5, horiz = F,cex=2.5)
mtext(side = 2, at = 2.5, text = "C", las = 2, cex = 2.5, family = "ArialN",line = 5.5)
...
pdf(file="02_fig/prop-05/fig-s5-d.pdf",,width=6,height=6)
par(mar = c(5, 4.5, 4, 3))
...
filled.contour(x= seq(0, 3, length.out = nrow(z)),
               ...
               key.axes=axis(4,family = "ArialN",cex.axis=1.7,seq(-2,1,by=1)),
...
pdf(file="02_fig/prop-05/fig-s5-e.pdf",width=6,height=6)
par(mar = c(5, 4.5, 4, 3))
...
filled.contour(x= seq(0, 3, length.out = nrow(z)),
               ...
               z,
               ylim = range(y, finite = TRUE),
               xlab = "Lag, months", ylab = "Adjusted vaccine coverage", main = "Effect on evenness of lineage",
               col = cols,level=levels,
               key.title=title("",cex.main=1.7,las=1,family = "ArialN"),
               family = "ArialN",cex.main=1.7,cex.lab=1.7,
               key.axes=axis(4,family = "ArialN",cex.axis=1.7,seq(-1,0.5,by=0.5)),
               plot.axes = { axis(1,cex.axis=1.7, at = 0:nlag, c(0:nlag),family = "ArialN") 
                 axis(2,cex.axis=1.7,family = "ArialN")})
abline(h = 70, lty = 5,lwd=1)
dev.off()
