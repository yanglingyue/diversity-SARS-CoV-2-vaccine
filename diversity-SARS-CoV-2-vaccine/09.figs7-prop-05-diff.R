...
model_si <-glm(sub_lineage ~ Lag(data$sub_lineage,1)+travel_si_cb+gr_cb+npi_si_cb+NI_diff+FBW_si_cb+NPI:F_I_BWP,family=gaussian,data=data)
summary(model_si)
...
cp_FI_gr <- crosspred(FBW_gr_cb, model_gr, cen=0,at=0:100,by=0.1,bylag=0.1,cumul=TRUE) #slices
...
pdf("02_fig/prop-05/figs7-a-c.pdf", width = 18, height = 6)
...
pdf(file="02_fig/prop-05/fig-s7-d.pdf",,width=6,height=6)
par(mar = c(5, 4.5, 4, 3))
...
pdf(file="02_fig/prop-05/fig-s7-e.pdf",width=6,height=6)
par(mar = c(5, 4.5, 4, 3))
...
filled.contour(x= seq(0, 3, length.out = nrow(z)),
               ...
               key.title=title("",cex.main=1.7,las=1,family = "ArialN"),
               family = "ArialN",cex.main=1.7,cex.lab=1.7,
               key.axes=axis(4,family = "ArialN",cex.axis=1.7,seq(-3.5,0.5,by=1)),
               plot.axes = { axis(1,cex.axis=1.7, at = 0:nlag, c(0:nlag),family = "ArialN") 
                 axis(2,cex.axis=1.7,family = "ArialN")})
abline(h = 70, lty = 5,lwd=1)
dev.off()
