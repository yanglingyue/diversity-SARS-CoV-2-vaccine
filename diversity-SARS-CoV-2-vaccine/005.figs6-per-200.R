...
data <-read.csv("01_data/subsample_per_200_lineage_diversity.csv")
...
plot(varby, gr[,lag_gr], col = col1, type = "l", lwd = 3, 
     xlab = "Stringency index", ylab = "", main = "", 
     ylim = range(-10, 0), frame.plot = F, axes = F,family = "ArialN",cex.lab=2.8,line =5)
axis(1, at = 0:5*20, labels = 0:5*20,cex.axis=2.8,family = "ArialN",mgp=c(3,2,0))
axis(2,at=c(-5:0*2),cex.axis=2.8,family = "ArialN",mgp=c(3,1,0))
xx <- c(varby, rev(varby))
yy <- c(gr.lci[,lag_gr], rev(gr.uci[,lag_gr]))
polygon(xx, yy, col = tcol1, border = tcol1)
abline(h = 0, lty = 3)
...
pdf(file="02_fig/figs6-per-200_g.pdf",width=7.5,height=7.5)
par(mar = c(5, 4.5, 4, 3))
...
pdf(file="02_fig/figs6-per-200_h.pdf",width=7.5,height=7.5)
par(mar = c(5, 4.5, 4, 3))
...
filled.contour(x= seq(0, 3, length.out = nrow(z)),
               ...
               family = "ArialN",cex.main=2,cex.lab=1.8,
               key.axes=axis(4,family = "ArialN",cex.axis=1.8,las=1,seq(-3.6,0.4,by=1)),
               plot.axes = { axis(1,cex.axis=1.8, at = 0:nlag, c(0:nlag),family = "ArialN") 
                 axis(2,cex.axis=1.8,family = "ArialN",las=3) })
abline(h = 70, lty = 5,lwd=2.5)
...
pdf(file="02_fig/figs6-per-200_i.pdf",width=7.65,height=8.4)
