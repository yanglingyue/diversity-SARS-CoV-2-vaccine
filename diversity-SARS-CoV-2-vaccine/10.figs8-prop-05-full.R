...
cp_FI_gr <- crosspred(FI_gr_cb, model_gr, cen=0,at=0:100,by=0.1,bylag=0.1,cumul=TRUE) #slices
...
pdf("02_fig/prop-05/figs8-a-c.pdf", width = 18, height = 6)
...
pdf(file="02_fig/prop-05/fig-s8-d.pdf",,width=6,height=6)
par(mar = c(5, 4.5, 4, 3))
...
filled.contour(x= seq(0, 3, length.out = nrow(z)),
               ...
               key.axes=axis(4,family = "ArialN",cex.axis=1.7,seq(-9,3,by=3)),
...
pdf(file="02_fig/prop-05/fig-s8-e.pdf",width=6,height=6)
par(mar = c(5, 4.5, 4, 3))
...
filled.contour(x= seq(0, 3, length.out = nrow(z)),
               ...
               z,
               ylim = range(y, finite = TRUE),
               xlab = "Lag, months", ylab = "Adjusted vaccine coverage", main = "Effect on diversity",
               col = cols,level=levels,
               key.title=title("",cex.main=1.7,las=1,family = "ArialN"),
               family = "ArialN",cex.main=1.7,cex.lab=1.7,
               key.axes=axis(4,family = "ArialN",cex.axis=1.7,seq(-4,4,by=2)),
               plot.axes = { axis(1,cex.axis=1.7, at = 0:nlag, c(0:nlag),family = "ArialN") 
                 axis(2,cex.axis=1.7,family = "ArialN")})
abline(h = 70, lty = 5,lwd=1)
dev.off()
