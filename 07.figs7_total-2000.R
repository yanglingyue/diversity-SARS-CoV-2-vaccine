...
mtext(side = 2, at = 1.52, text = "A", las = 2, cex = 2.5, family = "ArialN",line = 5.5)
...
plot(varby, gr[,lag_gr], col = col1, type = "l", lwd = 3, 
     xlab = "International travel", ylab = "", main = "", 
     ylim = range(-6, 12), frame.plot = F, axes = F,family = "ArialN",cex.lab=2.8,line =5)
axis(1,at=0:5*3,cex.axis=2.8,family = "ArialN",mgp=c(3,2,0))
axis(2,at=c(-1:0*6,2:0*6),cex.axis=2.8,family = "ArialN",mgp=c(3,1,0))
xx <- c(varby, rev(varby))
yy <- c(gr.lci[,lag_gr], rev(gr.uci[,lag_gr]))
polygon(xx, yy, col = tcol1, border = tcol1)
abline(h = 0, lty = 3)
...
polygon(xx, yy, col = tcol2, border = tcol2)
abline(h = 0, lty = 3)
mtext("Effect on growth rate of cases and \nrichness of lineage diversity",side = 2,at=3, line =4,cex=1.9,family = "ArialN")
...
plot(varby, gr[,lag_gr], col = col1, type = "l", lwd = 3, 
     xlab = "Stringency index", ylab = "", main = "", 
     ylim = range(-8, 4), frame.plot = F, axes = F,family = "ArialN",cex.lab=2.8,line =5)
axis(1, at = 0:5*20, labels = 0:5*20,cex.axis=2.8,family = "ArialN",mgp=c(3,2,0))
axis(2,at=c(-2:0*4,1:0*4),cex.axis=2.8,family = "ArialN",mgp=c(3,1,0))
xx <- c(varby, rev(varby))
yy <- c(gr.lci[,lag_gr], rev(gr.uci[,lag_gr]))
polygon(xx, yy, col = tcol1, border = tcol1)
abline(h = 0, lty = 3)
...
polygon(xx, yy, col = tcol2, border = tcol2)
abline(h = 0, lty = 3)
mtext("Effect on growth rate of cases and \nrichness of lineage diversity",side = 2,at=-2, line =4,cex=1.9,family = "ArialN")
...
pdf(file="02_fig/figs7-2000_g.pdf",width=7.5,height=7.5)
par(mar = c(5, 4.5, 4, 3))
...
filled.contour(x= seq(0, 3, length.out = nrow(z)),
               ...
               key.axes=axis(4,family = "ArialN",cex.axis=1.8,las=1,seq(-3,1,by=1)),
...
pdf(file="02_fig/figs7-2000-_h.pdf",width=7.5,height=7.57)
par(mar = c(5, 4.5, 4, 3))
...
pdf(file="02_fig/figs7-2000_i.pdf",width=7.65,height=8.4)
...
# Plot
#par(mar = c(5, 4, 4, 4))
plot(gr_low_cp,"slices",lag=3,cumul=T,col="black",ylim=c(-14,14),xlim=c(-5,10),axes=F,ann=F,lwd=3,cex.axis=1.9,
...
axis(2,at=c(-2:0*7,2:0*7),cex.axis=1.8,family = "ArialN",las=3,mgp=c(3,1,0))
...
mtext("Effect on Shannon's index\n of lineage diversity",side = 2,at=0, line =3,cex=1.8,family = "ArialN")
