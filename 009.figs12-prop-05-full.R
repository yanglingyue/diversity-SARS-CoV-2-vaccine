...
V1 <- data$F_I # for adjusted vaccine coverage not corrected for vaccine waning and the immunity of boosters 
...
df <- data.frame(Y1, Y2, Y3, Y4, P, V1, N, C, M)



model_gr <-  gam(Y1 ~ npi_gr_cb+FI_gr_cb+lineage_gr_cb+travel_gr_cb+N+s(C, bs = "re")+P:V1
                 ,method = "REML",data=df)
summary(model_gr)
#AIC(model_gr)
...
cp_FI_gr <- crosspred(FI_gr_cb, model_gr, cen=0,by=0.1,at=0:100,bylag=0.1,cumul=TRUE) #slices


model_si <-gam(Y2 ~ Lag(df$Y2,1)+travel_si_cb+gr_cb+npi_si_cb+FI_si_cb+N+s(C, bs = "re")+P:V1,
               method = "REML",data=df)
summary(model_si)
...
cp_FI_si <- crosspred(FI_si_cb, model_si, cen=0,by=0.1,at=0:100,bylag=0.1,cumul=TRUE) #slices


model_gr_rich <- gam(Y1 ~ FI_gr_cb+richness_gr_cb+npi_gr_cb+travel_gr_cb+N+s(C, bs = "re")+P:V1,
                     method = "REML",data=df)
...
cp_FI_gr_rich <- crosspred(FI_gr_cb, model_gr_rich, cen=0,by=0.1,at=0:100,bylag=0.1,cumul=TRUE) #slices


model_si_rich <- gam(Y3 ~ Lag(df$Y3,1)+travel_si_cb+npi_si_cb+gr_cb+FI_si_cb+N+s(C, bs = "re")+P:V1,
                     method = "REML",data=df)
...
cp_FI_si_rich <- crosspred(FI_si_cb, model_si_rich, cen=0,by=0.1,at=0:100,bylag=0.1,cumul=TRUE) #slices


model_gr_even <- gam(Y1 ~ FI_gr_cb+evenness_gr_cb+npi_gr_cb+travel_gr_cb+N+s(C, bs = "re")+P:V1,
                     method = "REML",data=df)
...
cp_FI_gr_even <- crosspred(FI_gr_cb, model_gr_even, cen=0,by=0.1,at=0:100,bylag=0.1,cumul=TRUE) #slices


model_si_even <- gam(Y4 ~ Lag(df$Y4,1)+travel_si_cb+npi_si_cb+gr_cb+FI_si_cb+N+s(C, bs = "re")+P:V1,
...
cp_FI_si_even <- crosspred(FI_si_cb, model_si_even, cen=0,by=0.1,at=0:100,bylag=0.1,cumul=TRUE) #slices
AIC(model_si_even)
...
pdf("02_fig/figs12-full-a-f.pdf", width = 21, height = 21/3*2)
...
plot(varby, gr[,lag_gr], col = col1, type = "l", lwd = 3, 
     xlab = "International travel", ylab = "", main = "", 
     ylim = range(0, 24), frame.plot = F, axes = F,family = "ArialN",cex.lab=2.8,line =5)
axis(1,at=0:5*3,cex.axis=2.8,family = "ArialN",mgp=c(3,2,0))
axis(2,at=c(4:0*6),cex.axis=2.8,family = "ArialN",mgp=c(3,1,0))
xx <- c(varby, rev(varby))
yy <- c(gr.lci[,lag_gr], rev(gr.uci[,lag_gr]))
polygon(xx, yy, col = tcol1, border = tcol1)
abline(h = 0, lty = 3)
...
polygon(xx, yy, col = tcol2, border = tcol2)
abline(h = 0, lty = 3)
mtext("Effect on growth rate of cases and \nrichness of lineage diversity",side = 2,at=12, line =4,cex=1.9,family = "ArialN")
...
plot(varby, gr[,lag_gr], col = col1, type = "l", lwd = 3, 
     xlab = "Stringency index", ylab = "", main = "", 
     ylim = range(-15, 5), frame.plot = F, axes = F,family = "ArialN",cex.lab=2.8,line =5)
axis(1, at = 0:5*20, labels = 0:5*20,cex.axis=2.8,family = "ArialN",mgp=c(3,2,0))
axis(2,at=c(-3:0*5,1:0*5),cex.axis=2.8,family = "ArialN",mgp=c(3,1,0))
xx <- c(varby, rev(varby))
yy <- c(gr.lci[,lag_gr], rev(gr.uci[,lag_gr]))
polygon(xx, yy, col = tcol1, border = tcol1)
abline(h = 0, lty = 3)
...
polygon(xx, yy, col = tcol2, border = tcol2)
abline(h = 0, lty = 3)
mtext("Effect on growth rate of cases and \nrichness of lineage diversity",side = 2,at=-5, line =4,cex=1.9,family = "ArialN")
...
legend("bottomleft",
    ...
    lwd = 3, lty = 1, bty = "n", 
       y.intersp = 1.5, horiz = F,cex=2.4)
mtext(side = 2, at = 10, text = "F", las = 2, cex = 2.5, family = "ArialN",line = 5.5)
...
pdf(file="02_fig/figs12-full-g.pdf",width=7.5,height=7.5)
par(mar = c(5, 4.5, 4, 3))
...
filled.contour(x= seq(0, 3, length.out = nrow(z)),
               ...
               key.axes=axis(4,family = "ArialN",cex.axis=1.8,las=1,seq(-6,0,by=1)),
...
pdf(file="02_fig/figs12-full-h.pdf",width=7.5,height=7.5)
par(mar = c(5, 4.5, 4, 3))
...
filled.contour(x= seq(0, 3, length.out = nrow(z)),
               ...
               family = "ArialN",cex.main=2,cex.lab=1.8,
               key.axes=axis(4,family = "ArialN",cex.axis=1.8,las=1,seq(-1,0.2,by=0.3)),
               plot.axes = { axis(1,cex.axis=1.8, at = 0:nlag, c(0:nlag),family = "ArialN") 
                 axis(2,cex.axis=1.8,family = "ArialN",las=3) })
abline(h = 70, lty = 5,lwd=2.5)
...
pdf(file="02_fig/figs12-full-i.pdf",width=7.65,height=8.4)
...
# Plot
#par(mar = c(5, 4, 4, 4))
plot(gr_low_cp,"slices",lag=3,cumul=T,col="black",ylim=c(-18,6),xlim=c(-5,10),axes=F,ann=F,lwd=3,cex.axis=2,
...
axis(2,at=c(-3:0*6,1:0*6),cex.axis=1.8,family = "ArialN",las=3,mgp=c(3,1,0))
...
mtext("Effect on Shannon's index of lineage diversity",side = 2,at=-6, line =3,cex=1.8,family = "ArialN")
