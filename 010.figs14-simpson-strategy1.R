...
dir.create("02_fig/figs14_simpson")
outpath <-  "02_fig/figs14_simpson/simpson-figs14"
...
Y2  <- data$simpson_lineage # response variable (Simpson's index of lineage diversity based on the subsampling strategy 1)
...
model_gr <-  gam(Y1 ~ npi_gr_cb+FBW_gr_cb+simpson_gr_cb+travel_gr_cb+N+s(C, bs = "re")+P:V
                 ,method = "REML",data=df)
summary(model_gr)
AIC(model_gr)
...
#r_l <- cp_travel_si$cumlow["1.635",]/si_m 
#r_f <- cp_travel_si$cumfit["1.635",]/si_m 
#r_h <- cp_travel_si$cumhigh["1.635",]/si_m 

#0.09753737/si_m 
pdf(file=paste0(outpath,"_a-b.pdf"), width = 16, height = 8)
#pdf("02_fig/fig2.pdf", width = 21, height = 21/3*2)

par(mfrow=c(1,2),mar = c(8, 8, 5, 2),oma = c(0, 2, 0, 0))
...
plot(varby, gr[,lag_gr], col = col1, type = "l", lwd = 3, 
     ...
     ylim = range(-3, 1), frame.plot = F, axes = F,family = "ArialN",cex.lab=2,line =5)
axis(1,at=0:5*3,cex.axis=2,family = "ArialN",mgp=c(3,2,0))
axis(2,at=c(-3:0*1,1:0*1),cex.axis=2,family = "ArialN",mgp=c(3,1,0))
...
abline(h = 0, lty = 3)
mtext("Effect on growth rate of cases and \nSimpson's index of lineage diversity",side = 2,at=-1, line =4,cex=1.7,family = "ArialN")

legend("bottomleft",
       legend = c(paste0("Simpson's index of lineage diversity"),
                  paste0("Growth rate of cases")),
       col = c(col2, col1), 
       lwd = 3, lty = 1, bty = "n", 
      y.intersp = 1.5, horiz = F,cex=1.75)
mtext(side = 2, at = 1.5, text = "A", las = 2, cex = 1.75, family = "ArialN",line = 5.5)
...
plot(varby, gr[,lag_gr], col = col1, type = "l", lwd = 3, 
     ...
     ylim = range(-4, 0), frame.plot = F, axes = F,family = "ArialN",cex.lab=2,line =5)
axis(1, at = 0:5*20, labels = 0:5*20,cex.axis=2,family = "ArialN",mgp=c(3,2,0))
axis(2,at=c(-4:0*1),cex.axis=2,family = "ArialN",mgp=c(3,1,0))
xx <- c(varby, rev(varby))
yy <- c(gr.lci[,lag_gr], rev(gr.uci[,lag_gr]))
polygon(xx, yy, col = tcol1, border = tcol1)
abline(h = 0, lty = 3)
...
polygon(xx, yy, col = tcol2, border = tcol2)
abline(h = 0, lty = 3)
mtext("Effect on growth rate of cases and\nSimpson's index of lineage diversity",side = 2,at=-2, line =4,cex=1.7,family = "ArialN")
...
legend("bottomleft",
       ...
       lwd = 3, lty = 1, bty = "n", 
      y.intersp = 1.5, horiz = F,cex=1.75)
mtext(side = 2, at = 0.42, text = "B", las = 2, cex = 1.75, family = "ArialN",line = 5.5)
...
p1  <- ggplot(result_vero_shannon,aes(x=coverage, y=vero_fit,group=coverage))+ 
  ...
  labs(x="Adjusted vaccine coverage (%)", y= "Effect on Simpson's index of lineage diversty" )+  
...
ggsave(file=paste0(outpath,"-c",".pdf"),
...
ggsave(file=paste0(outpath,"_d",".pdf"),
...
ggsave(file=paste0(outpath,"_Simpson-index-growth-rate",".pdf"),
       p,width = 16,height =8)
