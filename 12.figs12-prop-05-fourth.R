model_gr <-  glm(GR_NCM_log_diff ~ npi_gr_cb+FBW_gr_cb+redefine_gr_cb++travel_gr_cb+NI_diff+NPI:F_I_BWP,family=gaussian,data=data)
summary(model_gr)

model_si <-glm(sub_redefine ~ Lag(data$sub_redefine,1)+travel_si_cb+gr_cb+npi_si_cb+FBW_si_cb+NI_diff+NPI:F_I_BWP,family=gaussian,data=data)
summary(model_si)
...
cp_lineage_gr <- crosspred(redefine_gr_cb, model_gr,at=0:40/10,cen=0,by=0.1,bylag=0.1,cumul=TRUE) #slices
...
model_gr_rich <- glm(GR_NCM_log_diff ~ FBW_gr_cb+redefine_richness_gr_cb+npi_gr_cb+travel_gr_cb+NI_diff+NPI:F_I_BWP,family=gaussian,data=data)
summary(model_gr_rich)

model_si_rich <- glm(sub_richness_redefine ~ Lag(data$sub_richness_redefine,1)+travel_si_cb+npi_si_cb+gr_cb+FBW_si_cb+NI_diff+NPI:F_I_BWP,family=gaussian,data=data)
...
model_gr_even <- glm(GR_NCM_log_diff ~ FBW_gr_cb+redefine_evenness_gr_cb+npi_gr_cb+travel_gr_cb+NI_diff+NPI:F_I_BWP,family=gaussian,data=data)
summary(model_gr_even)

model_si_even <- glm(sub_evenness_redefine ~ Lag(data$sub_evenness_redefine,1)+travel_si_cb+npi_si_cb+gr_cb+NI_diff+FBW_si_cb+NPI:F_I_BWP,family=gaussian,data=data)
...
pdf("02_fig/figs12-a-f-fourth.pdf", width = 21, height = 21/3*2)
...
pdf(file="02_fig/figs12_g-fourth.pdf",width=7.5,height=7.5)
par(mar = c(5, 4.5, 4, 3))
...
pdf(file="02_fig/figs12_h-fourth.pdf",width=7.5,height=7.57)
par(mar = c(5, 4.5, 4, 3))
...
pdf(file="02_fig/figs12_i-fourth.pdf",width=7.65,height=8.4)
...
axis(2,at=c(-3:0*10,1:0*10),cex.axis=1.8,family = "ArialN",las=3,mgp=c(3,1,0))
...
mtext("Effect on Shannon's index\n of lineage diversity",side = 2,at=-10, line =3,cex=2,family = "ArialN")
