...
# Setting the lag for Shannon index's of lineage diversity, merging the sub-lineages into lineages according to PANGO nomenclature# 
lag_redefine <- tsModel::Lag(data$sub_redefine, group = data$country, k = 0:nlag) 

# Setting the lag for the richness of lineage diversity, merging the sub-lineages into lineages according to PANGO nomenclature# 
lag_redefine_richness <- tsModel::Lag(data$sub_richness_redefine, group = data$country, k = 0:nlag)

# Setting the lag for the evenness of lineage diversity, merging the sub-lineages into lineages according to PANGO nomenclature# 
lag_redefine_evenness <- tsModel::Lag(data$sub_evenness_redefine, group = data$country, k = 0:nlag)


# Define cross basis matrix (combining nonlinear exposure and hysteresis function)
var <- lag_redefine_richness
redefine_richness_gr_cb <- crossbasis(var,
                                      argvar = list(fun = "poly",degree=3),
                                      arglag = list(fun = "ns",df=2))
summary(redefine_richness_gr_cb)


var <- lag_redefine_evenness
redefine_evenness_gr_cb <- crossbasis(var,
                                      argvar = list(fun = "poly",degree=3),
                                      arglag = list(fun = "ns",df=2))
summary(redefine_evenness_gr_cb)


var <- lag_redefine
redefine_gr_cb <- crossbasis(var,
                             argvar = list(fun = "poly",degree=3),
                             arglag = list(fun = "ns",df=2))
summary(redefine_gr_cb)
...
Y2  <- data$sub_redefine # response variable (Shannon's index of lineage diversity based on the subsampling strategy 1,merging the sub-lineages into lineages according to PANGO nomenclature)
Y3  <- data$sub_richness_redefine # response variable (the richness of lineage diversity based on the subsampling strategy 1, merging the sub-lineages into lineages according to PANGO nomenclature)
Y4  <- data$sub_evenness_redefine # response variable (the evenness of lineage diversity based on the subsampling strategy 1,merging the sub-lineages into lineages according to PANGO nomenclature)
...
model_gr <-  gam(Y1 ~ npi_gr_cb+FBW_gr_cb+redefine_gr_cb+travel_gr_cb+N+s(C, bs = "re")+P:V
                 ,method = "REML",data=df)
summary(model_gr)
#AIC(model_gr)
...
model_gr_rich <- gam(Y1 ~ FBW_gr_cb+redefine_richness_gr_cb+npi_gr_cb+travel_gr_cb+N+s(C, bs = "re")+P:V,
                     method = "REML",data=df)
...
model_gr_even <- gam(Y1 ~ FBW_gr_cb+redefine_evenness_gr_cb+npi_gr_cb+travel_gr_cb+N+s(C, bs = "re")+P:V,
                     method = "REML",data=df)
...
pdf("02_fig/figs15-fourth-a-f.pdf", width = 21, height = 21/3*2)
...
mtext(side = 2, at = 2.45, text = "A", las = 2, cex = 2.5, family = "ArialN",line = 5.5)
...
plot(varby, gr[,lag_gr], col = col1, type = "l", lwd = 3, 
     xlab = "International travel", ylab = "", main = "", 
     ylim = range(0, 20), frame.plot = F, axes = F,family = "ArialN",cex.lab=2.8,line =5)
axis(1,at=0:5*3,cex.axis=2.8,family = "ArialN",mgp=c(3,2,0))
axis(2,at=c(4:0*5),cex.axis=2.8,family = "ArialN",mgp=c(3,1,0))
xx <- c(varby, rev(varby))
yy <- c(gr.lci[,lag_gr], rev(gr.uci[,lag_gr]))
polygon(xx, yy, col = tcol1, border = tcol1)
abline(h = 0, lty = 3)
...
polygon(xx, yy, col = tcol2, border = tcol2)
abline(h = 0, lty = 3)
mtext("Effect on growth rate of cases and \nrichness of lineage diversity",side = 2,at=10, line =4,cex=1.9,family = "ArialN")
...
plot(varby, gr[,lag_gr], col = col1, type = "l", lwd = 3, 
     xlab = "Stringency index", ylab = "", main = "", 
     ylim = range(-16, 4), frame.plot = F, axes = F,family = "ArialN",cex.lab=2.8,line =5)
axis(1, at = 0:5*20, labels = 0:5*20,cex.axis=2.8,family = "ArialN",mgp=c(3,2,0))
axis(2,at=c(-4:0*4,1:0*4),cex.axis=2.8,family = "ArialN",mgp=c(3,1,0))
xx <- c(varby, rev(varby))
yy <- c(gr.lci[,lag_gr], rev(gr.uci[,lag_gr]))
polygon(xx, yy, col = tcol1, border = tcol1)
abline(h = 0, lty = 3)
...
polygon(xx, yy, col = tcol2, border = tcol2)
abline(h = 0, lty = 3)
mtext("Effect on growth rate of cases and \nrichness of lineage diversity",side = 2,at=-6, line =4,cex=1.9,family = "ArialN")
...
legend("bottomleft",
    ...
    lwd = 3, lty = 1, bty = "n", 
       y.intersp = 1.5, horiz = F,cex=2.4)
mtext(side = 2, at = 6.7, text = "F", las = 2, cex = 2.5, family = "ArialN",line = 5.5)
...
pdf(file="02_fig/figs15-fourth-g.pdf",width=7.5,height=7.5)
par(mar = c(5, 4.5, 4, 3))
...
pdf(file="02_fig/figs15-fourth-h.pdf",width=7.5,height=7.5)
par(mar = c(5, 4.5, 4, 3))
...
filled.contour(x= seq(0, 3, length.out = nrow(z)),
               ...
               family = "ArialN",cex.main=2,cex.lab=1.8,
               key.axes=axis(4,family = "ArialN",cex.axis=1.8,las=1,seq(-3.3,0.2,by=0.7)),
               plot.axes = { axis(1,cex.axis=1.8, at = 0:nlag, c(0:nlag),family = "ArialN") 
                 axis(2,cex.axis=1.8,family = "ArialN",las=3) })
abline(h = 70, lty = 5,lwd=2.5)
...
pdf(file="02_fig/figs15-fourth-i.pdf",width=7.65,height=8.4)
...
# Plot
#par(mar = c(5, 4, 4, 4))
plot(gr_low_cp,"slices",lag=3,cumul=T,col="black",ylim=c(-21,7),xlim=c(-5,10),axes=F,ann=F,lwd=3,cex.axis=1.9,
...
axis(2,at=c(-3:0*7,1:0*7),cex.axis=1.8,family = "ArialN",las=3,mgp=c(3,1,0))
...
mtext("Effect on Shannon's index of lineage diversity",side = 2,at=-7, line =3,cex=2,family = "ArialN")
